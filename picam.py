...
def main():
    with picamera.PiCamera() as camera:
        ...
        while True:
            ...
            camera.capture(frame, 'bgr') # Capture a frame from the camera
            process_frame(frame)
...
def draw_boxes(frame,boxes,indexes,class_ids):
    ...
    for i in indexes:
        if classes[class_ids[i]] in targets: # check if the detected object's class is in the updated targets list
